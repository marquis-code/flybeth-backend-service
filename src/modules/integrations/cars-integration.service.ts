...
export class CarsIntegrationService {
    ...
    constructor(
        private sabreCarsProvider: SabreCarsProvider,
        private providerConfigService: ProviderConfigService,
        @Inject(CACHE_MANAGER) private cacheManager: Cache,
    ) {
        this.registerAdapter(sabreCarsProvider);
    }
    ...
    async search(query: CarSearchQuery): Promise<{
        ...
        // We'll prioritize Sabre as requested
        const activeProviderNames = ["sabre"];
        ...
        const promises = activeProviderNames
            ...
            .map((adapter) =>
                adapter!
                    .searchCars(query)
                    .then((results) => {
                        this.logger.log(`${adapter!.providerName} returned ${results.length} results`);
                        return results;
                    })
                    .catch((err) => {
        ...
        // Sort by price
        withCommission.sort((a, b) => a.priceWithCommission - b.priceWithCommission);
