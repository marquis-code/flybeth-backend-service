...
export class ChatGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  async handleMessage(
    @ConnectedSocket() client: Socket,
    @MessageBody()
    payload: { roomId: string; content: string; receiverId?: string },
  ) {
    ...
    // Aggressive role-based notifications
    const senderRole = client.data.user.role;

    if (senderRole === "customer") {
      // Notify all admins/staff about customer message
      this.server
        .to("role:super_admin")
        ...
        .emit("notification", {
          ...
          content: `New support message from customer: ${client.data.user.email}`,
